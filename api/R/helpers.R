...
fetch_team_season_aggregate <- function(conn, team_id, stat_key, season) {
  ...
  params <- list(team_id = team_id, season = season, stat = stat_key)
  result <- query_rows(conn, query, params)
...
fetch_player_season_aggregate <- function(conn, player_id, stat_key, season) {
  if (stat_key == "points") {
    ...
    query <- paste(
      "SELECT",
      "  SUM(CASE WHEN stat = 'goal1' THEN match_value ELSE 0 END +",
      "      2 * COALESCE((SELECT COALESCE(SUM(CASE WHEN stat = 'goal2' THEN match_value ELSE 0 END), 0)",
      "                   FROM player_match_stats pms2",
      "                   WHERE pms2.player_id = pms1.player_id AND pms2.match_id = pms1.match_id), 0)) AS total,",
      "  COUNT(DISTINCT CASE WHEN stat = 'goal1' THEN match_id END) AS games",
      "FROM player_match_stats pms1",
      "WHERE pms1.player_id = ?player_id",
      "  AND pms1.season = ?season"
    )
    params <- list(player_id = player_id, season = season)
  } else {
    query <- paste(
      "SELECT",
      "  SUM(COALESCE(match_value, 0)) AS total,",
