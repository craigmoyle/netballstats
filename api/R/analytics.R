...
player_analytics_match_rows <- function(conn, metric_key, seasons = NULL, team_id = NULL, round = NULL, search = "") {
  ...
  query <- paste(
    ...
    "  CASE",
    "    WHEN ?metric_key = 'playerScoringEfficiency' THEN ROUND((player_base.goal1 + 2 * player_base.goal2) / NULLIF(player_base.goal_attempts, 0), 4)",
    "    WHEN ?metric_key = 'playerAttackInvolvementRate' THEN ROUND((player_base.feeds + player_base.goal_assists + player_base.centre_pass_receives + player_base.goal_attempts) / NULLIF(team_context.team_attacking_actions, 0), 4)",
    "    WHEN ?metric_key = 'playerTurnoverCostRate' THEN ROUND((player_base.general_play_turnovers + player_base.unforced_turnovers + player_base.intercept_pass_thrown) / NULLIF(player_base.feeds + player_base.goal_assists + player_base.centre_pass_receives + player_base.goal_attempts, 0), 4)",
    "    WHEN ?metric_key = 'playerDefensiveDisruption' THEN ROUND(player_base.gain + player_base.intercepts + player_base.deflections + player_base.rebounds, 4)",
    "    WHEN ?metric_key = 'playerPressureBalance' THEN ROUND((player_base.gain + player_base.intercepts + player_base.deflections + player_base.rebounds) - (player_base.general_play_turnovers + player_base.unforced_turnovers + player_base.intercept_pass_thrown), 4)",
    "    ELSE NULL",
    "  END AS metric_value",
    "FROM player_base",
...
team_analytics_match_rows <- function(conn, metric_key, seasons = NULL, team_id = NULL, round = NULL) {
  query <- paste(
    ...
    "  CASE",
    "    WHEN ?metric_key = 'teamFinishingEfficiency' THEN ROUND((goal1 + 2 * goal2) / NULLIF(goal_attempts, 0), 4)",
    "    WHEN ?metric_key = 'teamBallSecurityRate' THEN ROUND((general_play_turnovers + unforced_turnovers) / NULLIF(possessions, 0), 4)",
    "    WHEN ?metric_key = 'teamDisruption' THEN ROUND(gain + intercepts + deflections, 4)",
    "    WHEN ?metric_key = 'teamPossessionControlBalance' THEN ROUND((gain + intercepts) - (general_play_turnovers + unforced_turnovers), 4)",
    "    ELSE NULL",
    "  END AS metric_value",
    "FROM team_base",
    "WHERE 1 = 1"
